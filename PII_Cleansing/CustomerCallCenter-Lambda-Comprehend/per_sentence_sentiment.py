...
def get_overall_label_score(pred_sent_dict, method, label_dict=label_dict):
    ...
    '''
    all_sent_labels = [sent_dict['label'] for sent_dict in pred_sent_dict.values()]
    if method == 'strict':
        if all([label==label_dict['pos'] for label in all_sent_labels]): # If all sentences predicted positive -- assign pos
            overall_sentiment = label_dict['pos']
        elif any([label==label_dict['neg'] for label in all_sent_labels]): # If any sentence is predicted negative -- assign neg
            overall_sentiment = label_dict['neg']
        else:
            overall_sentiment = label_dict['neutral']
        scores = [sent_dict['score'] for sent_dict in pred_sent_dict.values() if sent_dict['label'] == overall_sentiment]
	...
	if GENERATE_PER_SENTENCE_SENTIMENT:
		...
		if split_line_data and len(split_line_data)>1:
			...
			for sentence in split_line_data:
				sentiment_value, sentiment_score = comprehend_content_sentiment (comprehend, sentence) 
				pred_sent_dict[idx] = {'label': sentiment_value,'score': sentiment_score}
				#print("partial")
				idx+=1
			if pred_sent_dict:
				method='majority-neu' # #'majority-neg' 'strict'
				aggregated_sentiment_value, aggregated_sentiment_score = get_overall_label_score(pred_sent_dict, method)
