...
cleaned_bucket_name = "customercallcenterpiicleaned"
...
def lambda_handler(event, context):
    ...
    if "_standard" in s3filename:
        ...
        # Extract Content
        full_transcript = my.read_output_standard(json_content, CHANNEL_ID_AGENT, CHANNEL_ID_CUSTOMER)
        ...
        response = s3client.Bucket(s3bucket).put_object(Key=s3_path, Body=full_transcript, ContentType='text/plain')
            # since this results are uncleaned, we will store them in uncleaned bucket
    
        # Performing cleaning on Full Transcript text
        my.perform_pi_redaction_full_transcript(comprehend, s3client, s3filename, cleaned_bucket_name, full_transcript)
        ...
        my.construct_final_output(comprehend, s3client, cleaned_bucket_name, s3filename, system_filename, json_content, CHANNEL_ID_AGENT, CHANNEL_ID_CUSTOMER)
        ...
        my.perform_pi_redaction_callanalytics(comprehend, s3client, s3filename, cleaned_bucket_name, json_content)
